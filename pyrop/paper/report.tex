...
do not work on realistic binaries (ROPC), x86 64-bit binaries (mona.py), or are not fully featured (ROPgadget).
...
\begin{table*}[t,s]
  \begin{center}
    ...
    ArithmeticStoreG & AddrReg, InReg & Offset, $\diamond_b$ & M[AddrReg + Offset] $\diamond_b$$\leftarrow$ InReg \\
      \hline
    \end{tabular}
    \caption{Gadget Types that our Gadget Classifier can find.  M[addr] means accessing memory at the address addr and $\diamond_b$ means an arbitrary binary operation.}
  \label{gadgetTypes}
  \end{center}
\end{table*}
...
In our brief analysis and examples (described in Section \ref{examples}), we have not seen a gadget that has been misclassified using only the concrete emulation.
Thus, to improve the ROP compilation speed, our implementation does not verify gadgets.
Any of the necessary metadata (such as which registers are clobbered by a gadget) is extracted from the symbolic formula.
...
For example, to write a value to memory, the scheduler looks for a LoadMemG gadget to set the address, another LoadMemG gadget to set the value, and a StoreMemG to perform the write to memory.
...
Next, the scheduler adds the offset from this base function to the target function in the library.
...
As this vulnerability is a file format bug, there is little chance for the attacker to leak information about the memory layout of rsync (as compared to a network reachable vulnerability).
...
exploits, such as those needed in environments that do not allow remapping of memory permissions\cite{grsecurity}.
