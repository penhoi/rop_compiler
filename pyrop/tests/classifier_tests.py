...
class ClassifierTests(unittest.TestCase):
    ...
    def test_gadget_design(self):
        ...
        #Test cases:
        #1. NopG: no operation: designed for finding ifgadget
        #TRUE: ret
        code = '\xc3'
        self.assert_contain_this_type(code, NOP)
        
        #2. JumpG1: IP = AddrReg + Offset: designed for invoking other gadgets
        #TRUE: jmp eax
        ...
        #FALSE: push ebx; ret          
        code = "\xff\xe0"
        self.assert_contain_this_type(code, Jump1)
        code = "\x58\x59\x53\xc3"
        self.assert_contain_this_type(code, Jump1)
        code = "\x5b\xff\xd0"
        self.assert_contain_this_type(code, Jump1)
        code = "\xff\xe4"
        self.assert_not_contain_this_type(code, Jump1)
        code = "\x53\xc3"
        self.assert_not_contain_this_type(code, Jump1)
        ...
        #10. JumpG2: IP = AddrReg + Offset: designed for invoking Funcall gadgets.
        #TRUE: push ebx; ret
        #FALSE: pop eax; pop ecx; push ebx; ret
        #FALSE: jmp eax
        code = '\x53\xc3'
        self.assert_contain_this_type(code, Jump2)
        code = '\x58\x59\x53\xc3'
        self.assert_not_contain_this_type(code, Jump2)
        code = '\xff\xe0'
        self.assert_not_contain_this_type(code, Jump2)
        
        #10. MemJumpG: IP = [AddrReg + Offset]: designed for finding more jump gadgets.
        #TRUE: jmp [eax]
        #TRUE: pop ebx; call [eax]
        #FALSE: call [eax]
        #FALSE: ret
        code = '\xff\x20 '
        self.assert_contain_this_type(code, MemJump)
        code = '\x5b\xff\x10'
        self.assert_contain_this_type(code, MemJump)
        code = '\xff\x10'
        self.assert_not_contain_this_type(code, MemJump)
        code = '\xc3'
        self.assert_not_contain_this_type(code, MemJump)
        ...
        code = '\xff\xd0'
        self.assert_contain_this_type(code, RegFunCall)
        code = '\xff\xe0'
        self.assert_not_contain_this_type(code, RegFunCall)
        code = '\x5b\xff\xd0'
        self.assert_not_contain_this_type(code, RegFunCall)
        ...
        code = '\xff\x10'
        self.assert_contain_this_type(code, MemFunCall)
        code = '\xff\x20 '
        self.assert_not_contain_this_type(code, MemFunCall)
        code = '\x5b\xff\x10'
        self.assert_not_contain_this_type(code, MemFunCall)
        code = '\xc3'
        self.assert_not_contain_this_type(code, MemFunCall)
    ...
    def test_x86(self):
        tests = [
            ...
            ({NOP:1}, '\xc3'), #ret
            ({RegFunCall : 1}, '\xff\xd0'),       #call   *%eax
            ({Jump1 : 1}, '\xff\xe0'),       #jmp  *%eax
            ({MemFunCall : 1}, '\xff\x16'),   #call   *(%esi)
            ({MemFunCall : 1}, '\xff\x52\x04'),      #call   *0x4(%edx)
            ({RegStackSwitch:1}, '\xc9\xc3'), #leave; ret          
            ({RegStackSwitch:1}, '\x94\xc3'), #xchg eax, esp ; ret
            ({RegStackSwitch:1}, '\x89\xcc\xff\xe2'), #mov %ecx,%esp; jmp  *%edx
            ...
            ({AddConstGadget : 1, LoadMem:1}, '\x83\xc4\x08\x5b\xc3'),
            #add esp, 4 ; ret
            ({AddConstGadget : 1}, '\x83\xc4\x04\xc3'),
            #push %ebp;mov %esp,%ebp;sub $0x18,%esp;movl $0x804a03c,(%esp);call *%eax
            ({RegFunCall : 1}, '\x55\x89\xe5\x83\xec\x18\xc7\x04\x24\x3c\xa0\x04\x08\xff\xd0'),
            #longjmp: mov %esp, %ecx; jmp *%edx
            ({Jump1:1}, '\x89\xe1\xff\xe2'),
            #sub %eax, %ecx ; ret
    ...
    def test_amd64(self):
        tests = [
            ...
            ({Jump1 : 1, },   '\x5a\xfc\xff\xd0'),                                       # pop rdx, cld, call rax
            ...
            ({LoadMemJump: 1, Jump1:1}, '\x5d\xff\xe0'),           #pop %rbp; jmpq *%rax;
    ...
    def not_test_arm_jcc(self):
        tests = [
            ({Jump1: 2, JCC:1}, '\x00\x00S\xe3\x00\x00\x00\n\x13\xff/\xe1\x1e\xff/\xe1'),
            ({Jump1 : 1}, '\x13\xff\x2f\xe1'), # bx r3
            #({Jump1:2, JCC:1}, '\x03\xb1\x18\x47\x70\x47'),  #cbz r3, 10394 <register_tm_clones+0x28>; bx r3; bx lr
        ]
        #self.run_test(archinfo.ArchARM(), tests[2::1])
        arch = archinfo.ArchARM()
    ...
    def test_x86_jcc(self):
        tests = [
            ...
            ({MemFunCall : 1, NOP:1, JCC:1}, '\xff\x16\x72\xfc\xc3'),
            #ret ;mov $0x0,%eax;test %eax,%eax;je 80484bf <deregister_tm_clones+0xf>; 
            #push %ebp;mov %esp,%ebp;sub $0x18,%esp;movl $0x804a03c,(%esp);call *%eax
            ({NOP:1, RegFunCall : 5, JCC:1}, '\xc3\xb8\x00\x00\x00\x00\x85\xc0\x74\xf6\x55\x89\xe5\x83\xec\x18\xc7\x04\x24\x3c\xa0\x04\x08\xff\xd0'),
            #jb 4; call *(%esi); je 8 ; xor %eax,%eax; ret
            ({MemFunCall:1, LoadConst : 1, NOP:1, JCC:1}, '\x72\x02\xff\x16\x74\x02\x31\xc0\xc3'),
            #jb 4; call *(%esi); ret; je 8 ; xor %eax,%eax; ret
            ({MemFunCall:1, LoadConst : 1, NOP:2, JCC:1}, '\x72\x02\xff\x16\xc3\x74\x02\x31\xc0\xc3'),
            #ja 4; call *(%esi); call *%eax; ret 
            ({MemFunCall:1, RegFunCall : 2, NOP:1, JCC:1}, '\x77\x02\xff\x16\xff\xd0\xc3'),
            ...
            ({RegFunCall:1, LoadMem:3, LoadMultiple:1, JCC:2, NOP:1}, '\x8b\x06\x85\xc0\x74\x02\xff\xd0\x83\xc6\x04\x3b\xf7\x72\xf1\x5f\x5e\xc3'),
        ]
    ...
    def test_amd64_jcc(self):
        tests = [
            ...
            ({LoadMem:1, NOP:1, LoadMemJump:1, Jump1:3, JCC:1} ,'\x5d\xc3\xb8\x00\x00\x00\x00\x48\x85\xc0\x74\xf4\x5d\xbf\xb0\x20\x60\x00\xff\xe0'),
            #pop %rbp; retq; je 0; jmpq *%rax
            ({LoadMem:1, NOP:1, Jump1:2, JCC:1} , '\x5d\xc3\x74\xfc\xff\xe0'),
        ]
