...
class ValidatorTests(unittest.TestCase):
  ...
  def make_tests(self, arch, tests):
    code_gadget_list = []
    for (code, gadget_type, inputs, output, params, clobber, stack, ip, is_valid) in tests:
      input_regs = []
      ...
      if output != None:
        output = arch.registers[output][0]

      gadget = gadget_type(arch, 0x40000, input_regs, output, params, clobber, stack, ip)
  ...
  def test_amd64(self):
    ...
    tests = [
      ...
      ('\x48\x89\x03\xc3', StoreMem, ['rbx','rax'], None, [0], [], 8, 0, True), # mov QWORD PTR [rbx],rax; ret
      ('\x48\x89\x43\x08\xc3', StoreMem, ['rbx','rax'], None, [8], [], 8, 0, True), # mov QWORD PTR [rbx+0x8],rax; ret
      ('\x48\x89\x44\x24\x08\xc3', StoreMem, ['rsp','rax'], None, [8], [], 8, 0, True), # mov QWORD PTR [rsp+0x8],rax; ret
